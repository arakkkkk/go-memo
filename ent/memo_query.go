...
func (mq *MemoQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, memo.UsersTable, memo.UsersColumn),
		)
...
func (mq *MemoQuery) WithUsers(opts ...func(*UserQuery)) *MemoQuery {
	query := (&UserClient{config: mq.config}).Query()
...
func (mq *MemoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Memo, error) {
	...
	if query := mq.withUsers; query != nil {
		if err := mq.loadUsers(ctx, query, nodes,
			func(n *Memo) { n.Edges.Users = []*User{} },
			func(n *Memo, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MemoQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Memo, init func(*Memo), assign func(*Memo, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(memo.UsersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.memo_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "memo_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "memo_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (mq *MemoQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(memo.Table, memo.Columns, sqlgraph.NewFieldSpec(memo.FieldID, field.TypeUUID))
	_spec.From = mq.sql
